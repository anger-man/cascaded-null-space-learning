...
def init_weights(m):
    if isinstance(m, nn.Conv3d):
        nn.init.normal_(m.weight, mean=0.0, std=np.sqrt(2/(64 * m.in_channels)))
...
def plot_images(recon,net_out,full,rec,index,step):
    ...
    for j in range(3):
        ...
        im = ax[j,4].imshow(rec.cpu()[j,0])
        ax[j,4].axis('off')
        plt.colorbar(im,ax=ax[j,4])
        
    fig.tight_layout(pad=.1)
    plt.savefig('plots/%s/ep_%02d_test.png'%(index,step), dpi=100)
