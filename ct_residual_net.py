...
import optparse
from ct_models import init_weights, CascNullSpace, Unet, CascadedUnet
import pytorch_ssim
...
if options.arch == 'unet':
    net = Unet(n_channels =1,f_size=32,out_channels=1, out_acti = 'tanh')
else:
    net = CascadedUnet(n_channels =1,f_size=32,out_channels=1, out_acti='tanh')
...
for i in range(5):
    ...
    plt.axis('off')
    ax.imshow(np.abs(gts-preds)[i],cmap='Reds',vmin=0,vmax=.4)
plt.show()
...
stepsize = np.linspace(0.01,0.1,5)
...
for ss in stepsize:
    ss = ss if options.meth == 'residualIter' else 0
    psnr_list = []; ssim_list = []
    for k in K:
        gt = X[k]; metric =[]
