...
def P_ker(x,limited_mask,FP,rec_method):
    y = FP(x)
    y = y*limited_mask
    rec = rec_method(y)
    return (x - rec)
...
class CascNullSpace(nn.Module):
    def __init__(self, n_channels, f_size, limited_mask, FP, rec_method, normalization='none',
                 out_acti = 'linear', out_channels=1):
        super(CascNullSpace, self).__init__()
        
        self.lm = limited_mask;
        self.FP = FP
        self.rm = rec_method
        #unet block 1
    ...
    def forward(self, x):
        ...
        ###############################
        inter = x + P_ker(img_out0, self.lm, self.FP, self.rm)
        ...
        final = x + P_ker(img_out1, self.lm, self.FP, self.rm)
