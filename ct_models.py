...
class CascNullSpace(nn.Module):
    def __init__(self, n_channels, f_size, FP, rec_method, normalization='none',
                 out_acti = 'linear', out_channels=1 , single_nsblock = False):
        ...
        self.ds03 = downsampling(8*f_size, 8*f_size, normalization)
        self.dc04 = double_conv(8*f_size, 16*f_size, normalization)
        
        self.up03 = upsampling(16*f_size, 8*f_size, normalization)
    ...
    def forward(self, x):
        ...
        down04 = self.ds03(skip03)
        skip04 = self.dc04(down04)
        
        upsa03 = self.up03(skip04,skip03)
        upsa02 = self.up02(upsa03,skip02)
        upsa01 = self.up01(upsa02,skip01)
        obranch0 = self.up0_out(upsa01,skip00)
        img_out0 = self.out0(obranch0)
        
        ###############################
        inter = x + P_ker(img_out0, self.FP, self.rm)
        ...
        if self.single_nsblock:
            ...
            final = x + P_ker(img_out1,  self.FP, self.rm)
